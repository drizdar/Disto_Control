...
j = 3
while j == 3:
    noteq = raw_input("Type Yes or No: ").lower()
...
fl = 3
while fl == 3:
    tarq = raw_input("Type Yes or No: ").lower()
    if tarq == "yes" or tarq == "y":
        print "What is the difference (D_float) between the height of the water level and the target platform (put ? " \
              "if unknown)?. "
        tar = raw_input("D_float= ")
        unitx = raw_input("what are the units of D_float? ")
        tarr = str(tar)
        unitxx = str(unitx)
        print >> f, "D_float = ,%s ,%s " % (tarr, unitxx)
        print "What is the distance (D_base) from the laser to the bottom of the water column (put ? if unknown)?."
        tard = raw_input("D_base= ")
        unitz = raw_input("what are the units of D_base? ")
        tarrd = str(tard)
        unitzz = str(unitz)
        print >> f, "D_base = ,%s ,%s" % (tarrd, unitzz)
        print "Note: when performing analysis, make sure to subtract the measurements and D_float from D_base"
        print >> f, "Note: when performing analysis, make sure to subtract the measurements and D_float from D_base"
        fl = 1
...
while z == 3:
    stimeq = raw_input("Type Yes or No: ").lower()
    if stimeq == "yes" or stimeq == "y":
        print "Set a time"
        shourr = raw_input("Set an hour (24-hour time): ")
        sminutee = raw_input("Set a minute: ")
        shour = int(shourr)
...
while x == 3:
    etimeq = raw_input("Type Yes or No: ").lower()
    if etimeq == "yes" or etimeq == "y":
        print "set the time"
        emonthh = raw_input("set the month: ")
        edayy = raw_input("Set the day: ")
        ehourr = raw_input("Set the hour (24-hour time): ")
        eminutee = raw_input("Set the minute: ")
        emonth = int(emonthh)
        eday = int(edayy)
        ehour = int(ehourr)
...
print >> f, 'Note: Given measurement units signify most significant unit.'
...
print >> f, 'Date,Time,Measurement,Unit,DateTime (Use for Spreadsheet Analysis)'
...
while True:
    ...
    print >> f, '%s/%s/%s,%s:%s:%s,%f,%s,%s/%s/%s %s:%s:%s' % (
        now.month, now.day, now.year, now.hour, now.minute, now.second, number, unitText, now.month, now.day, now.year,
        now.hour, now.minute, now.second)
    print
    f.close()
